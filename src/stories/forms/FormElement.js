  ...
  return (
    <div className={["mb-4 w-full", className].join(" ")}>
      <div className="flex flex-row mb-2">
        ...
        {tooltipText && <Tooltip description={tooltipText} dark>
          <InformationCircleIcon className="text-textPlaceHolder hover:text-textBody ml-2 h-4 w-4"/>
        </Tooltip>}
      </div>
      ...
      {errorMessage && <div className="flex flex-row mt-0">
        ...
        <Typography type="caption" className="mt-2 text-formError" id={`${id}-error`}>
          {errorMessage}
        </Typography>
      </div>}
    </div>
